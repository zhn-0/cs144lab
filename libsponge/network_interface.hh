...
class NetworkInterface {
  private:
    ...
    std::map<uint32_t, std::pair<EthernetAddress, size_t>> IP2Ethernet{};
    std::map<uint32_t, size_t> has_broadcast{};

    class wait_to_send{
      public:
        InternetDatagram dgram;
        Address next_hop;
        wait_to_send(const InternetDatagram &_dgram, const Address &_next_hop): dgram(_dgram), next_hop(_next_hop) {}
    };

    std::queue<wait_to_send> waiting_ip{};
